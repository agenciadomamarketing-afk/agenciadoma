...
st.set_page_config(page_title="Chamados · Doma",page_icon="🎫",layout="wide")
...
st.caption(loja); st.markdown("---")

if df.empty: st.success("✅ Nenhum chamado registrado."); st.stop()
...
with c2: st.markdown(kpi("Em Aberto",str(abertos),alert=(abertos>10)),unsafe_allow_html=True)
...
st.dataframe(df[cols].sort_values("quantidade",ascending=False) if "quantidade" in df.columns else df[cols],use_container_width=True,hide_index=True)

if "motivo" in df.columns and "quantidade" in df.columns:
    st.markdown("---")
    fig=px.bar(df.sort_values("quantidade"),x="quantidade",y="motivo",orientation="h",
        color="status" if "status" in df.columns else None,
        color_discrete_map={"Aberto":"#C0392B","Em andamento":"#F5A623","Fechado":"#1DB954"},
        labels={"quantidade":"Chamados","motivo":""})
    fig.update_layout(paper_bgcolor="#1A1A1A",plot_bgcolor="#1A1A1A",
        font=dict(color="#FFFFFF"),height=320,margin=dict(l=20,r=20,t=20,b=20))
