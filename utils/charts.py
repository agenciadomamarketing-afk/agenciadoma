...
def kpi(label, value, d=None, alert=False):
    ...
    if d is not None:
        ...
        dh = '<span style="color:' + dc + ';font-size:12px">' + di + ' ' + f"{abs(d):.1%}" + ' vs mês ant.</span>'
    else:
        dh = ""
    return (
        ...
        '<div style="color:' + W + ';font-size:23px;font-weight:700;margin-bottom:4px">' + str(value) + '</div>' +
        dh + '</div>'
    )
...
def waterfall(labels, values):
    ...
    fig = go.Figure(go.Waterfall(
        ...
        increasing={"marker": {"color": G}},
        decreasing={"marker": {"color": E}},
...
def gauge(value, title, thresholds=(60, 80)):
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            "axis": {"range": [0, 100], "tickcolor": W},
            "bar": {"color": color}, "bgcolor": "#2D2D2D",
            "steps": [
